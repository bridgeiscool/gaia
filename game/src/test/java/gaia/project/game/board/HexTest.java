...
public class HexTest {
  ...
  public void adjacentHex() {
    Hex reference = Hex.emptyHex(new Coords(0, 0), 1);
    Hex adjacent = Hex.emptyHex(new Coords(3.0 * HEX_SIZE, ROOT_3 * HEX_SIZE), 2);
    Assert.assertEquals(
        adjacent,
  ...
  public void twoAwayFails() {
    Hex reference = Hex.emptyHex(new Coords(0, 0), 1);
    Hex adjacent = Hex.emptyHex(new Coords(6.0 * HEX_SIZE, 2 * ROOT_3 * HEX_SIZE), 2);
